...
class InhalerStepper extends ConsumerStatefulWidget {
  const InhalerStepper({super.key});

  @override
  _InhalerStepperState createState() => _InhalerStepperState();
}

class _InhalerStepperState extends ConsumerState<InhalerStepper> {
  int _currentStep = 0;
  String _name = '';
  double _strength = 0.01;
  String _strengthUnit = AppConstants.inhalerStrengthUnits[0]; // Default to mcg
  double _quantity = 1.0;
  String _quantityUnit = AppConstants.quantityUnits[4]; // Default to Puff
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'Name: $_name\n'
              'Type: Inhaler\n'
              'Strength: $_strength $_strengthUnit per puff\n'
              ...
              '${_addReferenceDose ? 'Reference Dose: $_referenceStrength $_strengthUnit ($_referencePuffs puffs)' : 'No Reference Dose'}',
        ),
        actions: [
          TextButton(
            ...
            onPressed: () async {
              try {
                ...
                final med = MedicationsCompanion(
                  name: Value(_name),
                  type: const Value('inhaler'),
                  strength: Value(_strength),
                  strengthUnit: Value(_strengthUnit),
                  quantity: Value(_quantity),
                  volumeUnit: Value(_quantityUnit),
                  referenceDose: Value(_addReferenceDose ? '$_referenceStrength $_strengthUnit ($_referencePuffs puffs)' : null),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Add Inhaler')),
      body: Column(
        children: [
              ...
              steps: [
                Step(
                  ...
                  content: Column(
                    children: [
                      ...
                      TextField(
                        decoration: const InputDecoration(labelText: 'Strength per Puff'),
                      ...
                      DropdownButton<String>(
                        ...
                        items: AppConstants.inhalerStrengthUnits
                ...
                Step(
                  ...
                  content: Column(
                    children: [
                      TextField(
                        decoration: const InputDecoration(labelText: 'Quantity (Puffs or Doses in Stock)'),
                        keyboardType: TextInputType.number,
                        onChanged: (value) {
                          ...
                          onChanged: (value) {
                            ...
                            setState(() {
                              ...
                              if (val != null && _strength > 0) {
                                _referencePuffs = val / _strength; // e.g., mcg to puffs
                              }
                            });
                          },
                        ),
                        TextField(
                          decoration: const InputDecoration(labelText: 'Number of Puffs'),
                          keyboardType: TextInputType.number,
                          onChanged: (value) {
                            final val = double.tryParse(value);
                            setState(() {
                              _referencePuffs = val;
                              if (val != null && _strength > 0) {
                                _referenceStrength = val * _strength; // puffs to mcg
                              }
                            });
                          },
                          controller: TextEditingController(
                            text: _referencePuffs?.toStringAsFixed(2) ?? '',
                          ),
                        ),
                      ],
                    ],
